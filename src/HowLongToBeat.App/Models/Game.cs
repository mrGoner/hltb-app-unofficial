...
public record Game(string Name, Uri? ImageUrl, TimeSpan MainDuration, TimeSpan ExtraDuration, TimeSpan PerfectDuration, int ReleaseYear)
{
    ...
    private static string ConvertToTimeString(TimeSpan timeSpan)
    {
        ...
        if (timeSpan.TotalMinutes < 60)
            return $"{Math.Round(timeSpan.TotalMinutes, 0, MidpointRounding.AwayFromZero)} Mins";

        if (timeSpan.Minutes is > 15 and < 45)
            return $"{(timeSpan.Days == 0 ? timeSpan.Hours : timeSpan.Days * 24 + timeSpan.Hours)}\u00bd Hours";

        return $"{Math.Round(timeSpan.TotalHours, 0, MidpointRounding.AwayFromZero)} Hours";
    }
}